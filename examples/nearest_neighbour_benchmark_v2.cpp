 ...
 *
 *
 *   Good memory practices are worth following — but it cannot rescue a bad algorithm.
...
//
//  What generative AI would write. Correct, but not optimized
//  - AoS layout: Point{x, y} packed together in memory.
//    Loading points[j].x fetches a cache line that also contains
//    points[j].y — only half the data we paid for is used right now.
...
static long long nearestNeighbour_SoA(const std::vector<Point>& points) {
    ...
    for (int i = 0; i < N; ++i) { sx[i] = points[i].x; sy[i] = points[i].y; }
...
__attribute__((noinline))
static long long nearestNeighbour_SoA_PreAlloc(const std::vector<Point>& points,
                                                NNWorkspace& ws) {
    const int N = static_cast<int>(points.size());

    // Reuse pre-allocated buffers — no heap allocation on the hot path
    for (int i = 0; i < N; ++i) { ws.sx[i] = points[i].x; ws.sy[i] = points[i].y; }
...
__attribute__((noinline))
static long long nearestNeighbour_Sorted_SoA(const std::vector<Point>& points,
                                              NNSearchState& state) {
    const int N = static_cast<int>(points.size());
...
int main() {
    ...
    auto r1 = benchmark("V1: Intuitive O(N²), AoS",
        [&](const std::vector<Point>& points){ return nearestNeighbour_Intuitive(points); },
        events);

    auto r2 = benchmark("V2: Intuitive O(N²), SoA            [+cache]",
        [&](const std::vector<Point>& points){ return nearestNeighbour_SoA(points); },
        events, r1.ms_per_event);

    auto r3 = benchmark("V3: Intuitive O(N²), SoA+pre-alloc  [+no heap]",
        [&](const std::vector<Point>& points){ return nearestNeighbour_SoA_PreAlloc(points, ws); },
        events, r1.ms_per_event);

    auto r4 = benchmark("V4: Sort+window, SoA+pre-alloc  [+O(Nk)]",
        [&](const std::vector<Point>& points){ return nearestNeighbour_Sorted_SoA(points, st); },
        events, r1.ms_per_event);
